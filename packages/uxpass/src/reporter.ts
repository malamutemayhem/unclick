...
function statusBadge(status: string): string {
  const colour = status === "ran" ? "#15803d" : "#64748b";
  return `<span style="display:inline-block;padding:2px 8px;border-radius:6px;background:${colour};color:#fff;font-size:12px;text-transform:uppercase;letter-spacing:.05em;">${escapeHtml(status)}</span>`;
}
...
export async function generateMarkdownReport(
  ...
  if (breakdown && typeof breakdown === "object" && Array.isArray(breakdown.critics)) {
    lines.push("## Critic coverage");
    lines.push("");
    lines.push("| Critic | Component | Mode | Status | Pass | Fail | N/A |");
    lines.push("| --- | --- | --- | --- | ---: | ---: | ---: |");
    for (const critic of breakdown.critics) {
      lines.push(
        `| ${critic.label} | ${critic.score_component} | ${critic.mode} | ${critic.status} | ${critic.pass} | ${critic.fail} | ${critic.na} |`,
      );
    }
    lines.push("");
  }
...
export async function generateHtmlReport(
  ...
  const criticRows = breakdown && typeof breakdown === "object" && Array.isArray(breakdown.critics)
    ? breakdown.critics
        .map((critic) => `<tr><td style="padding:8px;border-bottom:1px solid #e5e7eb;">${escapeHtml(critic.label)}</td><td style="padding:8px;border-bottom:1px solid #e5e7eb;font-family:ui-monospace,monospace;">${escapeHtml(critic.score_component)}</td><td style="padding:8px;border-bottom:1px solid #e5e7eb;">${escapeHtml(critic.mode)}</td><td style="padding:8px;border-bottom:1px solid #e5e7eb;">${statusBadge(critic.status)}</td><td style="padding:8px;border-bottom:1px solid #e5e7eb;text-align:right;color:#15803d;">${critic.pass}</td><td style="padding:8px;border-bottom:1px solid #e5e7eb;text-align:right;color:#b91c1c;">${critic.fail}</td><td style="padding:8px;border-bottom:1px solid #e5e7eb;text-align:right;color:#525252;">${critic.na}</td></tr>`)
        .join("\n")
    : "";
...
<body>
  ...
  ${criticRows ? `<h2>Critic coverage</h2>
  <table>
    <thead><tr><th>Critic</th><th>Component</th><th>Mode</th><th>Status</th><th style="text-align:right;">Pass</th><th style="text-align:right;">Fail</th><th style="text-align:right;">N/A</th></tr></thead>
    <tbody>${criticRows}</tbody>
  </table>` : ""}
  ...
  <p class="meta" style="margin-top:2rem;">Generated by UXPass deterministic runner. Browser-backed visual checks run when a visual snapshot is attached; queued critics are reserved for the LLM hat panel and Synthesiser chunks.</p>
</body>
</html>`;
}
