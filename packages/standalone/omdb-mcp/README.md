# OMDb Movies MCP by UnClick

Search movies and shows and get details by title or IMDb ID from the Open Movie Database.

> By UnClick. 180+ tools plus persistent agent memory in one install: https://unclick.world

## Install

Installs straight from GitHub, no npm account needed.

```json
{
  "mcpServers": {
    "omdb": {
      "command": "npx",
      "args": ["-y", "https://github.com/malamutemayhem/unclick/releases/download/standalone-mcps-latest/omdb.tgz"]
    }
  }
}
```

## Tools

- `omdb_search`
- `omdb_by_title`
- `omdb_by_id`

## Want the rest?

This is one connector. [UnClick](https://unclick.world) bundles 180+ tools plus
persistent cross-session agent memory in a single install.

## License

Apache-2.0
