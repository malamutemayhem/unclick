...
async function closePR(token: string, args: Record<string, unknown>): Promise<unknown> {
  const owner       = String(args.owner ?? "").trim();
  const repo        = String(args.repo  ?? "").trim();
  const pullNumber  = Number(args.pull_number);
  if (!token)                               return { error: "access_token is required to close a pull request." };
  if (!owner || !repo)                      return { error: "owner and repo are required." };
  if (!Number.isInteger(pullNumber) || pullNumber <= 0) return { error: "pull_number is required and must be a positive integer." };
  return githubFetch(
    token,
    "PATCH",
...
export async function githubAction(
  ...
  try {
    switch (action) {
      ...
      case "close_pr":
      case "close_pull_request": return closePR(token, args);
      ...
      default:
        return {
          error: `Unknown GitHub action: "${action}". Valid actions: search_repos, get_repo, list_issues, create_issue, list_prs, close_pull_request, get_user, list_gists, search_code.`,
        };
    }
  } catch (err) {
