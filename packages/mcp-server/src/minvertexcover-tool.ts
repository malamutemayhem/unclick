...
export async function minVertexCover(args: Record<string, unknown>) {
  ...
  for (const e of edges) {
    if (!Array.isArray(e) || e.length !== 2) {
      throw new Error("each edge must be a pair [u, v]");
    }
    const [u, v] = e;
    if (typeof u !== "number" || typeof v !== "number") {
      throw new Error("edge vertices must be numbers");
    }
    if (!Number.isInteger(u) || !Number.isInteger(v) || u < 0 || v < 0) {
      throw new Error("edge vertices must be non-negative integers");
    }
    if (u >= vertexCount || v >= vertexCount) {
      throw new Error("edge vertices must be less than vertex_count");
    }
    if (u === v) {
      throw new Error("self-loops are not allowed");
    }
    edgeMasks.push((1 << u) | (1 << v));
  }

  let bestMask = (1 << vertexCount) - 1;
  let bestSize = vertexCount;
  ...
  for (let i = 0; i < vertexCount; i++) {
    if (bestMask & (1 << i)) cover.push(i);
  }

  const meta: ConnectorMeta = {
    source: "local-computation",
    fetched_at: new Date().toISOString(),
    next_steps: ["Use minimum vertex cover for graph optimization problems"],
  };
