...
async function clockifyFetch(
  ...
  const CLOCKIFY_TIMEOUT_MS = Number(process.env.CLOCKIFY_TIMEOUT_MS) || 15000;
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), CLOCKIFY_TIMEOUT_MS);
  ...
  try {
    ...
    if (err instanceof Error && err.name === "AbortError") {
      return { error: `Clockify API request timed out after ${CLOCKIFY_TIMEOUT_MS}ms.` };
    }
