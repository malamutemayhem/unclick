...
const HEARTBEAT_PROTOCOL: HeartbeatProtocol = {
  ...
  procedure: [
    ...
    "Warm chat build rule: when a capable subscription chat seat is already holding fresh build context, DraftRoom is the first station and the default move is to build or fit the smallest safe draft immediately. Capture code, patch notes, tests, or a ScopePack in DraftRoom; defer to a low-capacity unattended runner only when the chat seat records an exact blocker and next build step.",
    "After check_signals, call save_conversation_turn with session_id='unclick-heartbeat-seat', role='assistant', and content containing the safe alert lines plus a brief progress summary and proof id only when there is an actionable diff, real progress, or proof. For delta-zero runs, skip the continuity write.",
    ...
    "If nudgeonly_receipt_bridge returns receipt_request or escalation_request, save its bridge_id and receipt_line in the continuity receipt and alert line. If it returns advisory_only for owner silence past TTL, treat that as a bridge gap and continue with deterministic owner lease expiry evidence instead of waiting on NudgeOnly.",
    ...
    "If no change, send nothing. Do not post or save 'UnClick healthy', 'nothing changed', 'no user action needed', or similar delta-zero text. If a heartbeat caller must use post_message, set suppress_noop_heartbeat=true so no-op heartbeat text returns a quiet success instead of a Boardroom message. If UnClick surfaces action_needed, blocker, failed check, stale ACK, or approval-required items, send only the alert format.",
