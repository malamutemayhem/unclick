...
export const CATALOG: ToolDef[] = [
  ...
  // MCP tools so agents discover the session protocol.
  {
    ...
    endpoints: [
      ...
      {
        id: "memory.save_schedule",
        name: "Save Schedule",
        description: "Save Schedule - Define a recurring job for the AutoPilot Runner: an exact endpoint call, a Playbook wake packet, or a reminder note, on an interval or daily/weekly time (timezone aware). Definitions only; an opt-in runner process executes them.",
        method: "POST",
        path: "/v1/memory/schedules/save",
        requiresAuth: false,
        inputSchema: {
          type: "object",
          properties: {
            name: { type: "string", description: "Short label, e.g. 'Monday GEOPass check'" },
            description: { type: "string" },
            cadence: {
              type: "object",
              properties: {
                every: { type: "string", description: "Interval like 30m, 4h, 1d (min 5m)" },
                daily_at: { type: "string", description: "24h HH:MM" },
                weekday: { type: "string", description: "monday..sunday, with daily_at for weekly" },
            ...
            action: {
              type: "object",
              properties: {
                kind: { type: "string", enum: ["endpoint", "playbook", "note"] },
            ...
            deliver_to: { type: "array", items: { type: "string" }, description: "Delivery hints for the runner, e.g. boardroom, telegram:<chat_id>, slack:<channel>" },
            enabled: { type: "boolean", default: true },
            tags: { type: "array", items: { type: "string" } },
            agent_id: { type: "string" },
            session_id: { type: "string" },
          },
          required: ["name", "cadence", "action"],
        },
      },
      {
        id: "memory.list_schedules",
        name: "List Schedules",
        description: "List Schedules - List saved recurring jobs with enabled state and next due time.",
        method: "POST",
        path: "/v1/memory/schedules/list",
        requiresAuth: false,
        inputSchema: {
          type: "object",
      ...
      {
        id: "memory.get_schedule",
        name: "Get Schedule",
        description: "Get Schedule - Fetch a schedule's full definition, cadence, action, delivery hints, and run stats.",
        method: "POST",
        path: "/v1/memory/schedules/get",
        requiresAuth: false,
        inputSchema: {
          type: "object",
      ...
      {
        id: "memory.list_due_schedules",
        name: "List Due Schedules",
        description: "List Due Schedules - Return enabled schedules whose next due time has passed. This is the runner's work queue.",
        method: "POST",
        path: "/v1/memory/schedules/due",
        requiresAuth: false,
        inputSchema: {
          type: "object",
          properties: { now: { type: "string", description: "Optional ISO timestamp for deterministic checks" } },
        },
      },
      {
        id: "memory.set_schedule_enabled",
        name: "Enable or Disable Schedule",
        description: "Enable or Disable Schedule - Toggle a schedule. Re-enabling re-anchors the next due time so no backlog fires.",
        method: "POST",
        path: "/v1/memory/schedules/enabled",
        requiresAuth: false,
        inputSchema: {
          type: "object",
          properties: {
      ...
      {
        id: "memory.record_schedule_run",
        name: "Record Schedule Run",
        description: "Record Schedule Run - Log a schedule execution outcome (success, failure, skipped) and advance the next due time. Failures emit a warning signal.",
        method: "POST",
        path: "/v1/memory/schedules/record-run",
        requiresAuth: false,
        inputSchema: {
          type: "object",
          properties: {
