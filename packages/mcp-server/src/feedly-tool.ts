...
async function feedlyFetch(
  ...
  const FEEDLY_TIMEOUT_MS = Number(process.env.FEEDLY_TIMEOUT_MS) || 15000;
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), FEEDLY_TIMEOUT_MS);
  ...
  try {
    ...
    if (err instanceof Error && err.name === "AbortError") {
      return { error: `Feedly API request timed out after ${FEEDLY_TIMEOUT_MS}ms.` };
    }
