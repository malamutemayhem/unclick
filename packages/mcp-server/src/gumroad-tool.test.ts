...
describe("gumroad connector resilience (L2)", () => {
  ...
  it("surfaces a clean rate-limit error on HTTP 429", async () => {
    ...
    await expect(gumroad_list_products({ access_token: "k" })).rejects.toThrow(/rate limit reached \(HTTP 429\).*retry after 30s/i);
  });
  ...
  it("surfaces a clean timeout error when the request aborts", async () => {
    ...
    await expect(gumroad_list_products({ access_token: "k" })).rejects.toThrow(/timed out/i);
  });

  it("validates input before any network call", async () => {
    await expect(gumroad_get_product({ access_token: "k" })).rejects.toThrow(/product_id is required/i);
  });
  ...
  it("maps the product list", async () => {
    ...
    const r = await gumroad_list_products({ access_token: "k" }) as Record<string, any>;
