...
test("Vercel auth infra comment names the missing bypass secret", () => {
  const body = buildTestPassPrCommentBody({
    failureKind: "infra_vercel_auth",
    httpCode: "000",
    authReason: "vercel_bypass_missing",
    howToFix: "Add the GitHub Actions secret VERCEL_AUTOMATION_BYPASS_SECRET.",
  });

  assert.match(body, /infra issue \(Vercel preview auth\)/);
  assert.match(body, /VERCEL_AUTOMATION_BYPASS_SECRET/);
  assert.match(body, /PR itself has not been evaluated/);
});
