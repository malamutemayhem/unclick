...
// Re-exported so existing imports (and api/chat.test.ts) keep working after
// the extraction into api/lib/chat-room-access.ts. The model endpoint receives
// thread_id from the browser, so it must re-check room access itself instead
// of trusting the earlier human-message append request.
export { resolveThreadPersistenceLane } from "./lib/chat-room-access.js";
