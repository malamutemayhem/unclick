...
On every subscription/chat-seat wake, before idling, run a QC pass over everything
the free autopilot agents (OpenHands / CodeRoom) shipped or attempted since the
last subscription wake. Read the diffs, PRs, and proof. If anything is wrong,
incomplete, or low-quality, the smart subscription seat fixes it immediately,
re-runs the checks, and repeats until clean.
