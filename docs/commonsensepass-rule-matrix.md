...
| R16 manual decision needed | `pass`, `done`, `merge_ready` | `HOLD` when the next step needs Chris or owner choice instead of more worker action. | Decision type, options, missing authorization, current owner. | Low: should be rare and explicit. | Orchestrator, Boardroom | Add decision-required evidence shape. |
