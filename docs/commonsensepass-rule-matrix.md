...
This matrix turns the current R1 to R6 implementation into worker-readable rules, then names the next candidate rules so future chips can extend the pass without guessing.
...
| R6 | `route` | `HOLD` when lane evidence is missing. `ROUTE` when the current worker is not the required specialist lane. `PASS` when the current lane matches. | Current lane, required lane, optional explicit route destination. | Medium: over-routing can slow simple work if the lane map is too broad. | XPass, Bench specialists, worker router | `packages/commonsensepass/src/rules.ts::checkR6` |
...
| Routing specialist work | Run R6 before keeping work in a general lane. Respect `ROUTE` by handing it to the named specialist lane. |
...
| --- | --- | --- | --- | --- | --- | --- |
| R7 protected surface gate | `merge_ready`, `done`, `pass` | `HOLD` or `BLOCKER` when the change touches secrets, billing, DNS, production data mutation, force push, credential rotation, or destructive SQL without explicit authorization. | Changed files, protected-surface labels, owner authorization receipt id. | Medium: filename matching can overflag migrations and docs. Needs explicit override receipt. | Safety, SecurityPass, Autopilot merge | Extend `ClaimContext` with `changed_files`, `protected_surfaces`, `authorization_receipts`. |
| R8 stale claim lease | `done`, `pass`, `merge_ready` | `HOLD` when the claiming seat has no fresh check-in or its claim is older than the policy window. | Claim timestamp, owner id, owner last seen timestamp, lease window. | Medium: offline but completed workers may be held until proof is posted. | Orchestrator, Boardroom | Extend `TodoSnapshot` with claim timestamps and lease expiry. |
| R9 active conflict guard | `pass`, `done`, `merge_ready` | `BLOCKER` when another fresh owner has a conflicting claim on the same job, PR, or files. | Current claim, active claims, owner timestamps, owned files. | Medium: claims can be stale or too broad. Needs freshness filter. | Boardroom, Job Runner | Add claim snapshot list to `ClaimContext`. |
| R10 missing ScopePack | `pass`, `done` | `HOLD` when work starts or completes without exact files, proof path, and stop conditions. | Todo scope pack, owned files, verification command, stop conditions. | Low: early scoping jobs may be intentionally broad. Allow `claim_type=scoping`. | Boardroom, Job Runner | Add `scope_pack` evidence to todo snapshots. |
| R11 proof freshness window | `pass`, `merge_ready`, `done` | `BLOCKER` when proof is older than the latest code or state change. | Proof timestamp, proof subject id, current head SHA or job updated timestamp. | Medium: non-code docs may not need full rerun. Needs proof kind. | Review, TestPass, PR workers | Add proof refs with `fetched_at` and `subject_revision`. |
| R12 reviewer independence | `merge_ready` | `HOLD` when builder and reviewer are the same seat and policy requires independent review. | Builder id, reviewer id, policy flag, review receipt. | Low: solo autopilot lanes may explicitly allow self-review. | Review, Safety | Extend PR snapshot with builder and reviewer ids. |
| R13 stale queue health | `healthy`, `quiet`, `no_work` | `BLOCKER` when queue health was computed from stale Boardroom, Jobs room, or Orchestrator reads. | Read timestamps, source freshness window, queue counts. | Medium: transient API lag could block a true quiet state. | Heartbeat, Orchestrator | Add source freshness metadata to R1 input. |
| R14 no-op status spam | `duplicate_wake`, `pass` | `SUPPRESS` when the new receipt says the same thing with the same proof id and no state delta. | New receipt fingerprint, prior receipt fingerprint, proof id, state delta. | Low: useful repeated proof may be suppressed unless state delta is included. | WakePass, Boardroom | Extend R3 beyond wakes to receipt fingerprints. |
| R15 specialist proof needed | `pass`, `done`, `merge_ready` | `ROUTE` when the claim needs a specialist proof lane before proceeding. | Touched surfaces, lane map, missing specialist proof. | Medium: over-routing can slow simple work. Needs narrow lane map. | XPass, Bench specialists | Extend R6 with surface-to-lane evidence. |
| R16 manual decision needed | `pass`, `done`, `merge_ready` | `HOLD` when the next step needs the operator or owner choice instead of more worker action. | Decision type, options, missing authorization, current owner. | Low: should be rare and explicit. | Orchestrator, Boardroom | Add decision-required evidence shape. |
...
| R6 | `route_current_lane_pass`, `route_missing_lane_hold`, `route_specialist_lane` |
