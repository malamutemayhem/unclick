...
**Status:** v0 design (creation-target named in ScopePack on UnClick todo `086847ea`).
**Authored 2026-05-15** by `claude-cowork-coordinator-seat` under any-worker-hat greenlight.
**Pairs with:** BuildBait v1 ladder Steps 9–12 (executor-gated portion of the crumb ladder), Claim-to-execute bridge v1 (`4ceb6c79`), CommonSensePass discipline (rank-36 in the consolidated automation methods doc).
...
A request is only honoured when BOTH gates evaluate to **passed** at the moment of execution.
...
- Implementation: the request's `heartbeat_tick_id` is compared against the latest tick at execution time; difference >1 cycle → block.
...
- `xpass_advisory: true` means the executor's PASS is advisory until a human reviewer + safety check sign off (per rank-10 gates). Setting this to false is reserved for explicit human-greenlit fast lanes.
- `intent: "test_only"` skips the code write step; the executor just runs the test command and posts the receipt — useful for proofing scoped freshness checks.
...
2. **Executors** — seats that can fulfil packets by writing code + opening PRs.
   - `pinballwake-build-executor` (lives on the same runner as the requester but enters from a separate entry point with broader authority — splits the surface).
   - Future: `claude-cowork-builder-seat` once the cowork side has a write-capable surface.
3. **Reviewers / Safety / Coordinator** — gated downstream of executor PASS; control merge.
   - `claude-cowork-seat` (Reviewer 🔍 — dormant since 2026-05-09; reassignment standing per Chris greenlight).
   - Future safety seat.
...
- Touches `.env`, `vercel.json`, `*.secret`, `*.key`, `.github/workflows/*`, `migrations/*`, `supabase/*.sql` without an explicit human ASSIGN with matching path scope.
- Force-pushes, resets, or deletes branches.
- Merges its own PRs. Merge is gated on Reviewer + Safety + Coordinator (per rank-10).
...
A packet that would cross any of these boundaries is held with `hold_reason: gate_blocked: protected_surface` and the originating ScopePack is flagged for human re-scope.
...
heartbeat tick                         → runner emits 0..1 executor_packet per todo
  → CommonSensePass evaluates packet   → PASS or HOLD receipt
    ...
    (PASS) → executor seat fulfils:
       ...
       → Reviewer/Safety/Coordinator gates handle merge downstream
```
...
- [ ] The lane refuses to merge its own PR; merge requires a Reviewer PASS comment from an allowlisted reviewer.
...
2. Create the missing one (this doc — now drafted in `_unclick-drafts/autopilot-executor-lane/`).
...
- No PR auto-merge — merge stays human-gated.
...
*This document is a draft on Chris's disk pending the Autopilot Executor Lane builder claim. If the live ScopePack on `086847ea` differs from anything stated here at integration time, the ScopePack wins — it's the contract.*
