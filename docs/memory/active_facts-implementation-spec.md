...
- **R4 partial confirmation**: high-value durable facts still win the very top slot in at least some cases, for example the operator's timezone at `#1`, but cron self-reports still push other real facts down into slots `#7` through `#9`
