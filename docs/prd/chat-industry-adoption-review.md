...
7. Human sender attribution: appended turns are signed with the account's email as `sender_id`, derived SERVER-SIDE from the verified session (a client-sent sender_id is ignored, so one member can never render as another). Another member's turns render left-aligned with their name; the viewer's own turns stay right-aligned. Legacy "you" rows keep the old rendering. Conscious call flagged by security review: the members endpoint exposes each member's `last_read_at` to other members, which is standard read-receipt behavior in mainstream chat clients and now carries real values.
