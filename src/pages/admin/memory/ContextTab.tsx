...
export default function ContextTab({ apiKey }: { apiKey: string }) {
  ...
  return (
    <div className="space-y-3">
            ...
            <div className="space-y-3">
              <div className="grid gap-3 md:grid-cols-2">
                <input
                  ...
                  className="rounded-md border border-white/[0.06] bg-white/[0.03] px-3 py-2 text-sm text-white placeholder:text-white/30 focus:border-amber-500/50 focus:outline-none"
                />
                <input
                  value={form.key}
                  disabled
              ...
              <textarea
                ...
                rows={3}
                placeholder="Value"
                className="w-full rounded-md border border-white/[0.06] bg-white/[0.03] px-3 py-2 text-sm text-white placeholder:text-white/30 focus:border-amber-500/50 focus:outline-none"
              />
              <div className="flex gap-2">
                <button onClick={handleUpdate} className="rounded-md bg-amber-500 px-3 py-1.5 text-xs font-semibold text-black hover:opacity-90">Save</button>
      ...
      {/* Add form */}
      {showForm ? (
        <div className="rounded-lg border border-amber-500/20 bg-white/[0.03] p-4 space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            <input
              value={form.category}
              onChange={(e) => setForm({ ...form, category: e.target.value })}
          ...
          />
          <div className="flex gap-2">
            <button onClick={handleCreate} className="rounded-md bg-amber-500 px-3 py-1.5 text-xs font-semibold text-black hover:opacity-90">Add Entry</button>
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg border border-dashed border-white/[0.1] py-3 text-xs text-white/40 transition-colors hover:border-amber-500/30 hover:text-amber-500"
        >
