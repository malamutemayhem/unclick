...
function CircleNavItem({ onClick }: { onClick?: () => void }) {
  const location = useLocation();
  const isCircle = location.pathname === "/admin/circle" || location.pathname.startsWith("/admin/circle/");
  const permissionsActive = location.pathname === "/admin/circle/permissions";
  ...
  return (
    <div>
      ...
      >
        ...
        <span className="flex-1">Circle</span>
        {isCircle
          ? <ChevronDown className="h-3 w-3 shrink-0" />
          : <ChevronRight className="h-3 w-3 shrink-0" />}
      </NavLink>
...
function SidebarNav({
  ...
  return (
    <>
      ...
      <CircleNavItem onClick={onLinkClick} />
