  ...
  connectorpass: [
    ...
    },
    {
      ...
      rows: [
        ...
        waiting("Live target is verified", "Environment checks run against the deployment that serves the public domain, not a preview or temporary project."),
        waiting("Env names match", "Start and callback code use the same client ID, client secret, and redirect URI names."),
        waiting("Provider accepts callback", "The provider authorization screen accepts the exact callback URL that production sends."),
        waiting("Provider trust screen is launch-ready", "Customer-facing login does not show an unverified, unsafe, or developer-personal warning unless that limitation is intentionally scoped."),
        waiting("Requested scopes match the tools", "The provider consent screen grants the read/write scopes that the shipped actions actually need."),
    ...
    },
    {
      ...
      rows: [
        ...
        waiting("Signed-in account matches stored key", "A browser login refuses to save credentials into a stale private key for another account."),
        ...
        waiting("Badges cover catalog fallbacks", "Apps added through a fallback registry still receive the same saved-connection status as catalog rows."),
  ...
  connectorpass: [
    {
      ...
      rows: [
        ...
        waiting("Provider start is proven", "The live provider login URL is checked far enough to catch callback mismatch before calling it ready."),
        ...
        waiting("Deployment target is proven", "The checked environment belongs to the deployment currently serving the public domain."),
