...
const OPTIONS = [
  ...
  { id: "h", label: "h", to: "/home-preview-h" },
  { id: "i", label: "i", to: "/home-preview-i" },
  ...
  { id: "r", label: "r", to: "/home-preview-r" },
  { id: "s", label: "s", to: "/home-preview-s" },
] as const;
...
export default function OptionRibbon({ active }: { active: OptionId }) {
  return (
    <div className="fixed bottom-4 left-1/2 z-50 w-auto max-w-[calc(100vw-1.5rem)] -translate-x-1/2">
      <div className="flex items-center gap-0.5 overflow-x-auto rounded-full border border-primary/30 bg-[#071e29]/95 p-1 shadow-[0_18px_50px_-18px_rgba(0,0,0,0.85)] backdrop-blur-md">
        ...
        {OPTIONS.map((option) => (
          <Link
            ...
            className={cn(
              "shrink-0 rounded-full px-2.5 py-1.5 font-mono text-[11px] uppercase tracking-[0.08em] transition-colors",
              option.id === active
